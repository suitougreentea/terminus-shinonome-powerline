...
#
# usage: jis2unicode encodings
#    or: jis2unicode -c (text conversion mode)
#
# convert jis charcode into encodings using MS CP932 translation table.
# or, if -c option is given convert EUC text into \u-notation text.
# (used for making name.src)
#
#	2002/2, by 1@2ch
#	* public domain *
#
...
0x5B, 0x005B, 
# 0x5C, 0x005C,
0x5C, 0x00A5,		# janapese YEN kludge
0x5D, 0x005D, 
0x5E, 0x005E, 
0x5F, 0x005F, 
0x60, 0x0060, 
...
if ($ARGV[0] eq '-c') {
	...
	chop;
	foreach my $c (split('')) {
	    my ($x) = unpack("n", $c) || ord($c);
	    my $u = $map{jis2sjis($x & 0x7f7f)};
	    if (32 <= $u && $u <= 126) {
		print chr($u);
	    } else {
		printf "\\u%04x", $u;
	...
	chop;
	if (/^encoding\s+(\d+)/i) {
	    $x = $map{jis2sjis(eval($1))};
	    if ($x) {
		print "ENCODING $x\n";
	    } else {
		print STDERR "jis2unicode: warning: undefined mapping: $1\n";
		print "ENCODING -1\n";
	    }
	} else {
	    print "$_\n";
